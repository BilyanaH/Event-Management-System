...
A Spring Boot 4 / Java 21 REST API for organising events, selling tickets, registering attendees, gathering feedback, and sending transactional emails.
...
5. [AI Feedback Summary](#ai-feedback-summary)
...
Create `src/main/resources/application-local.yml` (gitignored) with your local secrets. The `local` Spring profile is active by default:

```yaml
spring:
  ai:
    openai:
      api-key: YOUR_GROQ_API_KEY        # from console.groq.com
  mail:
    host: smtp.gmail.com                # or sandbox.smtp.mailtrap.io for testing
    port: 587
    username: your@gmail.com
    password: your-app-password         # Gmail App Password (16 chars)
    from: your@gmail.com
    properties:
      mail:
        smtp:
...
Other values have sensible defaults in `application.yml`:

| Variable | Default | Purpose |
|----------|---------|---------|
| `DB_URL` | `jdbc:mysql://localhost:3306/eventplatform?...` | MySQL JDBC URL |
...
| `GET` | `/events/{eventId}/feedback/ai-summary` | AI-generated feedback summary (public) |
...
Transactional emails are sent automatically via `JavaMailSender` on the following events:

| Trigger | Recipient | Description |
|---------|-----------|-------------|
| Registration confirmed | Attendee | Includes event details and confirmation code |
| Registration cancelled | Attendee | Cancellation acknowledgement |
| Check-in | Attendee | Check-in confirmation with timestamp |
| Forgot password | User | Reset link valid for 1 hour |

Configure SMTP in `application-local.yml`. Gmail App Passwords work out of the box. For testing use [Mailtrap](https://mailtrap.io) sandbox.

---

## AI Feedback Summary

`GET /events/{eventId}/feedback/ai-summary` returns an HTML summary of all submitted feedback generated by **Llama 3.3 70B** via [Groq](https://console.groq.com). The summary is structured as:

- **Highlights** — what attendees liked most
- **Room for improvement** — recurring complaints
- **Overall vibe** — general sentiment

Requires a valid `GROQ_API_KEY` in `application-local.yml`. The endpoint is public (no auth required).
